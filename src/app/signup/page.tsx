...
function SignUpForm() {
  ...
  const [step, setStep] = useState<"details" | "password" | "code">("details");
  ...
  useEffect(() => {
    // Preserve tracking params if needed
    const pkg = searchParams.get("package");
    if (pkg) {
      console.log("Sign up with package interest:", pkg);
    }
  }, [searchParams]);

  const handleContinue = (e: React.FormEvent) => {
    e.preventDefault();
    if (step === "details" && firstName && lastName && email) {
      setStep("password");
    ...
    <div className="flex flex-col items-center gap-8 w-full max-w-[400px]">
      <div className="flex flex-col items-center gap-2">
         ...
         <h1 className="text-2xl font-semibold tracking-tight">
            {step === "code" ? "Check your email" : "Sign up"}
         </h1>
      </div>
        ...
        <CardContent className="p-6 sm:p-8 flex flex-col gap-6">
          {step === "details" ? (
              <>
                  ...
                  {/* Apple Login excluded as per previous instructions */}
